...
public class GameState implements IGameState {
	...
	private float shrinkSpeed = 0.5f / 1000;
	...
	public void update(long timeElapsed) {
		...
		for (int i = 0; i < mPolygonModels.length; ++i) {
			float r = mPolygonModels[i].getRadius() - shrinkSpeed * timeElapsed;
			if (r < 0.1) {
				r = 6f;
		...
		playerAngle += inputHandler.currentPlayerAngluarDir() * playerSpeed
				* timeElapsed;
	...
	@Override
	public void onTouchEvent(float screenWidth, float screenHeight,
			final MotionEvent event) {
		inputHandler.handleMotionEvent(screenWidth, screenHeight, event);
	}
